...
:   PanelBase(inProcessor)
{
    ...
    mUpButton->setBounds(TC_DISPLAY_WIDTH + (2 * MEDIUM_BUFFER) + SMALL_BUFFER,
    ...
    mDownButton->setBounds(TC_DISPLAY_WIDTH + (2 * MEDIUM_BUFFER) + SMALL_BUFFER,
...
void TonalCenterPanel::paint(Graphics& g)
{
    ...
    // top button
    g.drawRect(TC_DISPLAY_WIDTH + (2 * MEDIUM_BUFFER),
               TC_LABEL_HEIGHT + (2 * MEDIUM_BUFFER),
               TC_BUTTON_DISPLAY_WIDTH,
               TC_BUTTON_DISPLAY_HEIGHT);
    
    // bottom button
    g.drawRect(TC_DISPLAY_WIDTH + (2 * MEDIUM_BUFFER),
               TC_LABEL_HEIGHT + (3 * MEDIUM_BUFFER) + TC_BUTTON_DISPLAY_HEIGHT,
               TC_BUTTON_DISPLAY_WIDTH,
               TC_BUTTON_DISPLAY_HEIGHT);
    
    // tonal center display
    g.drawRect(MEDIUM_BUFFER,
               TC_LABEL_HEIGHT + (2 * MEDIUM_BUFFER),
               TC_DISPLAY_WIDTH,
               TC_DISPLAY_HEIGHT);
}
